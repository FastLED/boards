# FastLED/boards — `platformio` branch

Slim mirror of platformio/platform-*/boards/*.json (Apache-2.0) and platform.json files.

## Status

**Initialized, no data yet.** First push by `sync-platformio.yml` will replace
...
Every nightly / manually-dispatched run of `.github/workflows/sync-platformio.yml`
...
rules. To change the data, edit the sync tool on `main` (`tools/sync_platformio.py`)
